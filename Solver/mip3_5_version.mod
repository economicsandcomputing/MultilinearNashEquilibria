...
minimize mip3: sum{i in Q, j in S[i]} (g[i,j]-(1-b[i,j]));
...
s.t. I6 {i in Q, j in S[i]}: r[i,j] <= U[i]*b[i,j];
s.t. I9 {i in Q, j in S[i]}: g[i,j] >= x[i,j];
s.t. I10 {i in Q, j in S[i]}: g[i,j] >= 1 - b[i,j];
s.t. I11 {i in Q, j in S[i]}: b[i,j]*b[i,j] = b[i,j];